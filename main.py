...
def get_args_parser():
    ...
    parser.add_argument('--total_frames', default=2e6, type=int, metavar='N',
                    help='for Kinetics, total frames similar to 1 WT video)')
...
def train_dino(args):
    ...
    dino_loss = DINOLoss(
        args.out_dim,
        args.local_crops_number,  # local_crops_number
        1,                        # global_crops_number
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, (images) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.amp.autocast('cuda', enabled=fp16_scaler is not None):
            ...
            # masked_img = utils.attention_masking(attn, args.patch_size images[:2])
            for msk in range(masked_img.shape[0]):
                student_masked_global.append(student(masked_img[msk,:], return_track= False))
            student_local = student(images[1:], return_track= False)
...
class DINOLoss(nn.Module):
    def __init__(self, out_dim, local_crop, global_crop, warmup_teacher_temp, teacher_temp,
    ...
    def forward(self, student_local, student_global, teacher_global, epoch):
        """
        Cross-entropy between softmax outputs of the teacher and student networks.
        """
        frame_loss = 0
        total_loss = 0
        num_frame = teacher_global.shape[0]
        for j in range(len(student_global)):
            student_masked_global = student_global[j]
            for cnt in range(num_frame):

                student_out = student_local[cnt,:]
                student_masked = student_masked_global[cnt,:]
                teacher_out = teacher_global[cnt,:]
                

                student_out = student_out / self.student_temp
                student_out = student_out.chunk(self.local_crop)


                student_masked = student_masked / self.student_temp
                student_masked = student_masked.chunk(self.global_crop)

                # teacher centering and sharpening
                temp = self.teacher_temp_schedule[epoch]
                teacher_out = F.softmax((teacher_out - self.center) / temp, dim=-1)
                teacher_out = teacher_out.detach().chunk(self.global_crop)
                ...
                for iq, q in enumerate(teacher_out):
                    for v in range(len(student_masked)):
                        loss = torch.sum(-q * F.log_softmax(student_masked[v], dim=-1), dim=-1)
                        total_mask_loss += loss.mean()
                        n_loss_terms += 1
                total_mask_loss /= n_loss_terms
                ...
                for iq, q in enumerate(teacher_out):
                    for v in range(len(student_out)):
                        ...
                        total_local_loss += loss.mean()
                        n_loss_terms += 1
                total_local_loss /= n_loss_terms

                frame_loss += (total_mask_loss + total_local_loss) * 0.5

            frame_loss /= num_frame
            total_loss += frame_loss

        total_loss /= len(student_global)
        self.update_center(teacher_global)
        return total_loss
    ...
    def update_center(self, teacher_global):
        ...
        teacher_global = rearrange(teacher_global, 'f b d -> (f b) d')

        batch_center = torch.sum(teacher_global, dim=0, keepdim=True)
        dist.all_reduce(batch_center)
        batch_center = batch_center / (len(teacher_global) * dist.get_world_size())
...
class DataAugmentationDINO(object):
    def __init__(self, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        self.local_transfo = transforms.Compose([
            transforms.ToPILImage(),
            transforms.RandomResizedCrop(112, scale=local_crops_scale, interpolation=Image.BICUBIC),
            flip_and_color_jitter,
    ...
    def __call__(self, image):
        ...
        tmp = frames.permute(1,0,2,3)
        for _ in range(self.local_crops_number):
            local = torch.zeros(global_1.shape[0], global_1.shape[1], global_1.shape[2] // 2, global_1.shape[3] // 2)
            for cnt in range(num_frame):
                local[cnt,:] = self.local_transfo(tmp[cnt,:])
